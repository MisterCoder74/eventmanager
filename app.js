...
const setupSuppliers = () => {
    ...
    const loadSuppliers = async () => {
        ...
        result.data.forEach(supplier => {
            ...
            row.innerHTML = `
                ...
                <td class="table-actions">
                    <button class="btn btn-sm btn-outline-primary" data-action="edit" data-id="${supplier.id}">Modifica</button>
                    <button class="btn btn-sm btn-outline-danger" data-action="delete" data-id="${supplier.id}">Elimina</button>
                </td>
            `;
            tableBody.appendChild(row);
        });
    ...
    tableBody.addEventListener('click', async (event) => {
        ...
        if (button.dataset.action === 'delete') {
            ...
            const result = await apiRequest('api_suppliers.php?action=delete', 'POST', { id });
            if (result.success) {
                loadSuppliers();
            } else {
                showMessage(result.error || 'Errore eliminazione', 'danger');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const data = Object.fromEntries(new FormData(form).entries());
        const action = data.id ? 'update' : 'create';
        const result = await apiRequest(`api_suppliers.php?action=${action}`, 'POST', data);
        if (result.success) {
            form.reset();
            modal.hide();
            loadSuppliers();
